...
importvcf <- function(infile){
  vcf_colnames <- c("chrom", "pos", "id", "ref", "alt", "qual", "filter", "info")
  vcf_coltypes <- readr::cols_only(chrom = "f",
                       pos = "i",
                       id = "c",
                       ref = "c",
                       alt = "c",
                       ...
                       filter = "c",
                       info = "c")
  readr::read_tsv(file = infile,
           col_names = vcf_colnames,
           comment = "#",
           col_types = vcf_coltypes) %>%
    mutate(
      type = case_when(
